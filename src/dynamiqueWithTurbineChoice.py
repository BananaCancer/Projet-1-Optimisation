...
puissances_turbines = [puissance_turbine_1, puissance_turbine_2, puissance_turbine_3, puissance_turbine_4, puissance_turbine_5]
...
def getListeEtatsParTurbine(turbines: list) -> Dict[int,List[int]]:
...
if __name__ == "__main__":
  # ====
  listeEtats = getListeEtatsParTurbine(turbines = TURBINES)
  # # ====
  debits_possible = list_debit_turbine()
  emptyStages = createStagesDF(turbines = TURBINES, listeEtats = listeEtats,
                              debits_possible = debits_possible)

  filledStages = {}
  for i, turbineID in enumerate(TURBINES[::-1]):
    if turbineID == TURBINES[-1]:
      filledStages[turbineID] = fillLastStage(stage = copy.copy(emptyStages[turbineID]),
                                      fonctionPuissance = puissances_turbines[turbineID - 1],
                                      turbineID=turbineID)
    else:
      filledStages[turbineID] = fillPreviousStages(stage=copy.copy(emptyStages[turbineID]),
                                           previousStage=copy.copy(filledStages[prevStage]),
                                           fonctionPuissance=puissances_turbines[turbineID - 1],
                                           turbineID=turbineID,
                                           previousTurbineID = prevStage,
                                           turbineIndex = NB_TURBINES - i)
    prevStage = turbineID
  dfs_choose = {}
  ...
  print(" ========================================= ")
  for i in TURBINES:
    print(f"turbine {i} : Q{i} = {dfs_choose[i]['X'+str(i)].values[0]}")
    print(dfs_choose[i])